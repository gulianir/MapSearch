...
public final class RatPoly {
	...
	public RatPoly(RatTerm rt) {
		...
		if (!rt.isZero()) {
			this.terms.add(new RatTerm(rt.getCoeff(), rt.getExpt()));
		}
		this.checkRep();
	}
	...
	private RatPoly(List<RatTerm> rt) {
		this.terms = rt;
		this.checkRep(); // to ensure rt satisfies rep invariant
	}
	...
	private static void incremExpt(List<RatTerm> lst, int degree) {
		// avoid going thru whole list if degree is 0
		if (!(degree == 0)) {
			for (int i = 0; i < lst.size(); i++) {
				lst.set(i, new RatTerm(lst.get(i).getCoeff(), lst.get(i).getExpt() + degree));
				// preserve rep invariant in case expt is negative after the
				// increment
	...
	private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
		// don't add newTerm if it's zero
		if (!newTerm.isZero()) {
			for (int i = 0; i < lst.size(); i++) {
				if (newTerm.getExpt() > lst.get(i).getExpt()) {
					// if we're greater than term at i then add at i and stop
					lst.add(i, newTerm);
					return;
				} else if (newTerm.getExpt() == lst.get(i).getExpt()) {
					// if the two terms have the same expt then replace term
					// at i with new term that has an updated coefficient
					...
					// end (avoid going further in the loop)
					return;
				}
			}
			// if we haven't returned yet then
			// degree (newTerm) > degree(this)
			// so add the newTerm at the end
			lst.add(newTerm);
		}
	...
	/**
	 * Duplicate all the terms in RatPoly; a "deep" clone
	 * 
	 * @return a duplication of all the RatPoly terms
	 */
	private ArrayList<RatTerm> dup() {
		ArrayList<RatTerm> temp = new ArrayList<RatTerm>();
		for (int i = 0; i < this.terms.size(); i++)
			temp.add(new RatTerm(this.terms.get(i).getCoeff(), this.terms.get(i).getExpt()));
		return temp;
	}
	...
	public RatPoly negate() {
		...
		ArrayList<RatTerm> temp = dup();
		RatPoly.scaleCoeff(temp, new RatNum(-1));
	...
	public RatPoly add(RatPoly p) {
		...
		ArrayList<RatTerm> temp = p.dup();
		for (RatTerm rt : this.terms)
			RatPoly.sortedInsert(temp, rt);
		return new RatPoly(temp);
	}
	...
	public RatPoly mul(RatPoly p) {
		...
		for (RatTerm rt : this.terms) {
			ArrayList<RatTerm> temp = p.dup();
			// multiply all the coefficients by rt's coefficient
			RatPoly.scaleCoeff(temp, rt.getCoeff());
			// add the exponent of rt to the exponents of all the terms
			RatPoly.incremExpt(temp, rt.getExpt());
	...
	public RatPoly div(RatPoly p) {
		...
		RatPoly quo = new RatPoly(); // quotient
		RatPoly rem = new RatPoly(this.dup()); // remainder

		// following algorithm laid out in pseudocode
		while (rem.terms.size() != 0 && rem.degree() >= p.degree()) {
			// divide leading terms: divide coefficients and subtract exponents
			RatNum c = rem.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff());
			RatPoly t = new RatPoly(new RatTerm(c, rem.degree() - p.degree()));
	...
	public RatPoly differentiate() {
		...
		ArrayList<RatTerm> temp = this.dup();
	...
	public double integrate(double lowerBound, double upperBound) {
		...
		RatPoly expr = this.antiDifferentiate(new RatNum(1));
		return expr.eval(upperBound) - expr.eval(lowerBound);
	}
